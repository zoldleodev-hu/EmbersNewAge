...
public class FieldChartBlockEntityRenderer implements BlockEntityRenderer<FieldChartBlockEntity> {
	...
	protected interface IChartSource {
	...
	@Override
	public void render(@NotNull FieldChartBlockEntity blockEntity, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
	...
	protected void renderChart(Level level, BlockPos pos, float x, float y, float z, VertexConsumer buffer, Matrix4f matrix4f, IChartSource source, Vector3f color1, Vector3f color2, Vector3f color3) {
    ...
    @Override
    public @NotNull AABB getRenderBoundingBox(@NotNull FieldChartBlockEntity tile) {
        return new AABB(Vec3.atLowerCornerWithOffset(tile.getBlockPos(), -1, 0, -1), Vec3.atLowerCornerWithOffset(tile.getBlockPos(), 2, 1, 2));
    }
}