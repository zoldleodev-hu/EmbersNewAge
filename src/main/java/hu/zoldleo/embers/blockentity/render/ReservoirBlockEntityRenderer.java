...
public class ReservoirBlockEntityRenderer implements BlockEntityRenderer<ReservoirBlockEntity> {
	...
	@Override
	public void render(@NotNull ReservoirBlockEntity blockEntity, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        //render fluid
    ...
    @Override
    public @NotNull AABB getRenderBoundingBox(@NotNull ReservoirBlockEntity tile) {
        return new AABB(Vec3.atLowerCornerWithOffset(tile.getBlockPos(), -1, 1, -1), Vec3.atLowerCornerWithOffset(tile.getBlockPos(), 2, 1 + tile.height, 2));
    }
}