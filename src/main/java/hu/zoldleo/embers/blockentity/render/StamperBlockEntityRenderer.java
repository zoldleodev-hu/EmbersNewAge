...
import hu.zoldleo.embers.blockentity.StamperBlockEntity;
...
public class StamperBlockEntityRenderer implements BlockEntityRenderer<StamperBlockEntity> {
	...
	@Override
	public void render(StamperBlockEntity blockEntity, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
    ...
    @Override
    public @NotNull AABB getRenderBoundingBox(@NotNull StamperBlockEntity tile) {
        return new AABB(Vec3.atLowerCornerWithOffset(tile.getBlockPos(), 0, -1, 0), Vec3.atLowerCornerWithOffset(tile.getBlockPos(), 1, 1, 1));
    }
}