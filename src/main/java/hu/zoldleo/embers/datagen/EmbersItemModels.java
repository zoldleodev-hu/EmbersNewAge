...
public class EmbersItemModels extends ItemModelProvider {
	...
	protected void registerModels() {
		...
		withExistingParent(RegistryManager.ALCHEMICAL_NOTE.getId().getPath(), ResourceLocation.fromNamespaceAndPath("neoforge", "item/default")).customLoader(SeparateTransformsModelBuilder::begin)
		.base(basicItem(Embers.res("alchemical_note_item")))
		.perspective(ItemDisplayContext.FIRST_PERSON_RIGHT_HAND, nested().parent(new ModelFile.UncheckedModelFile(ResourceLocation.parse("builtin/entity"))))
		.perspective(ItemDisplayContext.FIRST_PERSON_LEFT_HAND, nested().parent(new ModelFile.UncheckedModelFile(ResourceLocation.parse("builtin/entity"))))
		.perspective(ItemDisplayContext.GUI, nested().parent(new ModelFile.UncheckedModelFile(ResourceLocation.parse("builtin/entity"))))
		.perspective(ItemDisplayContext.FIXED, nested().parent(new ModelFile.UncheckedModelFile(ResourceLocation.parse("builtin/entity"))));
		...
		.texture("layer0", Embers.res("item/inflictor_gem"))
		.override().predicate(Embers.res("charged"), 1)
		.model(singleTexture("inflictor_gem_charged", ResourceLocation.parse("item/generated"), "layer0", Embers.res("item/inflictor_gem_charged")));
		...
		//itemWithTexture(RegistryManager.MITHRIL_ASPECTUS, "aspectus_mithril"); // TODO
        ...
        //itemWithTexture(RegistryManager.MITHRIL_INGOT, "ingot_mithril");
		...
		buttonInventory(RegistryManager.CAMINITE_BUTTON_ITEM.getId().getPath(), Embers.res("block/caminite_button"));
	}

	public void itemWithModel(DeferredItem<? extends Item> registryObject, String model) {
	...
	public void spawnEgg(DeferredItem<? extends Item> registryObject) {
		withExistingParent(registryObject.getId().getPath(), "item/template_spawn_egg");
	}

	public void itemWithTexture(DeferredItem<? extends Item> registryObject, String texture) {
		itemWithTexture(registryObject, "item/generated", texture);
	}

	public void itemWithTexture(DeferredItem<? extends Item> registryObject, String model, String texture) {
		ResourceLocation id = registryObject.getId();
		ResourceLocation textureLocation = ResourceLocation.fromNamespaceAndPath(id.getNamespace(), "item/" + texture);
		singleTexture(id.getPath(), ResourceLocation.parse(model), "layer0", textureLocation);
	}

	public void layeredItem(DeferredItem<? extends Item> registryObject, String model, String... textures) {
		ResourceLocation id = registryObject.getId();
	...
	public void bucketModel(DeferredItem<? extends BucketItem> registryObject, Fluid fluid) {
		ModelBuilder<ItemModelBuilder> builder = withExistingParent(registryObject.getId().getPath(), Embers.res("item/bucket_fluid"));
