...
public class EmberPacketEntity extends Entity {
	...
	public void affectTileEntity(BlockState state, BlockEntity blockEntity) {
		if (blockEntity instanceof IEmberPacketReceiver receiver && getEntityData().get(lifetime) > 1) {
			if (receiver.onReceive(this)) {
                IEmberCapability capability = level().getCapability(EmbersCapabilities.EMBER_CAPABILITY_BLOCK, blockEntity.getBlockPos(), null);
				if (capability != null) {
