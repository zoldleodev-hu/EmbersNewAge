package hu.zoldleo.embers.api.projectile;
...
public class ProjectileRay implements IProjectilePreset {
	...
	public void shoot(Level world) {
		...
		double distBlock = getPos().distanceToSqr(blockTrace.getLocation());
		...
		if (doContinue) { // TODO: why
			effect.onFizzle(world,newPosVector,this);
			impactDist = getPos().distanceTo(newPosVector);
		}

		if (!world.isClientSide) {
            PacketDistributor.sendToPlayersTrackingEntityAndSelf(shooter, new MessageEmberRayFX(startX,startY,startZ,dX,dY,dZ,impactDist,Misc.intColor(color),this.colorId));
		}
	}
}
