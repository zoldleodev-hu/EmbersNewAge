...
public class JEIPlugin implements IModPlugin {
	public static ResourceLocation pluginID = Embers.res("jei_plugin");
	...
	public static <C extends RecipeInput, T extends Recipe<C>> void addRecipes(IRecipeRegistration register, RecipeManager manager, RecipeType<T> jeiType, net.minecraft.world.item.crafting.RecipeType<T> type) {
		...
		for (RecipeHolder<T> holder : recipes) {
			if (holder.value() instanceof IVisuallySplitRecipe recipe) {
				visualRecipes.addAll(recipe.getVisualRecipes());
			} else {
				visualRecipes.add(holder.value());
			}
		}
	...
	public void registerRecipes(@NotNull IRecipeRegistration register) {
		...
		List<RecipeHolder<IMeltingRecipe>> meltingRecipeHolders = manager.getAllRecipesFor(RegistryManager.MELTING.get());

		List<IMeltingRecipe> meltingRecipes = new ArrayList<>();
		List<IMeltingRecipe> meltingBonusRecipes = new ArrayList<>();
		for (RecipeHolder<IMeltingRecipe> recipe : meltingRecipeHolders) {
            meltingRecipes.add(recipe.value());
			if (!recipe.value().getBonus().isEmpty())
				meltingBonusRecipes.add(recipe.value());
		}
